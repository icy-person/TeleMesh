...
impl TelegramService {
    pub async fn connect(api_id: i32, api_hash: String, session_path: &str) -> Result<Self> {
        ...
        let grammers_mtsender::SenderPool { runner, updates, handle } =
            SenderPool::new(Arc::clone(&session), api_id);
        let client = Client::new(handle);
        tokio::spawn(async move {
            if let Err(e) = runner.run().await { error!("Telegram runner stopped: {e}"); }
        });
        ...
        if service.client.is_authorized().await? {
            ...
            info!("Telegram session not authorized");
        }
            ...
            match c.stream_updates(updates, Default::default()).await {
                Ok(mut stream) => while let Some(update) = stream.next().await {
                    if let Ok(grammers_client::update::Update::NewMessage(message)) = update {
                        let _ = tx.send(Event::NewMessage(MessageDto {
                            id: message.id(),
    ...
    async fn peer_ref(&self, peer: &str) -> Result<grammers_session::types::PeerRef> {
        ...
        if let Ok(id) = value.parse::<i64>() {
            ...
            while let Some(d) = iter.next().await? {
                if d.peer_id().value() == id { return Ok(d.peer_ref()); }
            }
    ...
    pub async fn dialogs(&self, limit: usize) -> Result<Vec<DialogDto>> {
        ...
        while out.len() < limit {
            ...
            let p = d.peer();
            let last_message = d.last_message.as_ref().map(Self::message_dto);
            out.push(DialogDto {
                id: p.id().value(),
                name: p.name().unwrap_or_default().to_owned(),
                username: match p {
                    grammers_client::peer::Peer::User(ref u) => u.username().map(str::to_owned),
                    grammers_client::peer::Peer::Channel(ref c) => c.username().map(str::to_owned),
                    _ => None,
                },
                kind: match p {
    ...
    pub async fn login_start(&self, phone: &str, api_hash: &str) -> Result<grammers_client::client::LoginToken> {
        Ok(self.client.request_login_code(phone, api_hash).await?)
    }
    pub async fn login_code(&self, t: &grammers_client::client::LoginToken, code: &str)
        -> std::result::Result<grammers_client::peer::User, SignInError> {
        self.client.sign_in(t, code).await
    }
